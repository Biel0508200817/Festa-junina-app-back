...
// 1. Listar todos os itens de festa
...
// 3. Buscar itens de festa por categoria
...
app.put('/api/itens_festa/:id', async (req, res) => {
    ...
    if (error) return res.status(500).json({ error: error.message });
    if (!data.length) return res.status(404).json({ error: "Item de festa não encontrado." });

    res.json(data[0]);
});

// 6. Deletar item de festa
